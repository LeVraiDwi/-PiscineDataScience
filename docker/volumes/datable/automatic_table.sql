...
begin
    ...
    for me in SELECT ame FROM files loop
        ...
        execute FORMAT($cmd$ COPY %s (event_time, event_type, product_id, price, user_id, user_session) FROM '/tmp/customer/%s' DELIMITER ',' CSV HEADER; $cmd$, substring (me from 1 FOR strpos(me, '.') -1 ), me);
    end loop;
